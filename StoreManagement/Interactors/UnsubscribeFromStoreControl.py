...
class UnsubscribeFromStoreControl:
    """ invariants:
    """
    ...
    def execute(self) -> None:
        try:
            self._store.unsubscribe(self._subscriberID)
        except StoreIsDeactivatedException:
            raise RuntimeError('store is inactive' + str(self._store.getID()))
        except StoreSubscriberNonExistentException:
            raise RuntimeError('this user is not subscribed to this store')
